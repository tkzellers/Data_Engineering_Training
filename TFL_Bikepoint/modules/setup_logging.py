# Configure logs to retrieve INFO messages and higher
import os
import logging
from datetime import datetime

def setup_logging(logs_dir):
    """Sets up logging for all modules. Need to getlogger in every module separately.

    Args:
        logs_dir (): filepath to log directory you want to use

    Returns:
        logger: getlogger object
    """

    os.makedirs(logs_dir, exist_ok=True)
    log_filepath = os.path.join(logs_dir, (f"{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"))
    print(log_filepath)

    logging.basicConfig(
        level=logging.INFO, 
        format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', #'name' will take the name of the logger, which I set to be the name of the module using "logger = logging.getlogger(__name__)"
        filename=log_filepath
    )

    return logging.getLogger("main") #this initializes the logger when run in the main module, and sets the logger name for main to 'root'. 